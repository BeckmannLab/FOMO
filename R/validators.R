...
.validate_sample_metadata <- function(
    sample_metadata,
    has_genotype_matrix = FALSE
) {
    ...
    assert_that(
        ...
        msg = paste0(
            "'sample_metadata' is missing required column(s) ",
    ...
    if (has_genotype_matrix) {
        assert_that(
            ...
            msg = paste0(
                "'sample_metadata' should not contain a Genotype_Group_ID column if 'genotype_matrix' is provided"
            )
        )
    }

    duplicated_samples <- sample_metadata$Sample_ID[duplicated(
        sample_metadata$Sample_ID
    )]
    assert_that(
        length(duplicated_samples) == 0,
        msg = paste0(
            "'sample_metadata' has non-unique 'Sample_ID'(s) ",
...
.validate_genotype_matrix <- function(genotype_matrix, sample_metadata) {
    ...
    assert_that(
        ...
        msg = paste0(
            "'genotype_matrix' contains rowname(s) missing in colname(s) ",
            paste(rows_missing_in_cols, collapse = ", ")
        )
    )

    cols_missing_in_rows <- setdiff(
        colnames(genotype_matrix),
        rownames(genotype_matrix)
    )
    assert_that(
        ...
        msg = paste0(
            "'genotype_matrix' contains colname(s) missing in rowname(s) ",
            paste(cols_missing_in_rows, collapse = ", ")
        )
    )
    ...
    assert_that(
        ...
        msg = paste0(
            "'genotype_matrix' is not square, nrow=",
    ...
    assert_that(
        ...
        msg = paste0(
            "'genotype_matrix' contains NA value(s). If they represent samples with different genotypes, fill explicitly with zero or FALSE."
        )
    )
    ...
    assert_that(
        ...
        msg = paste0(
            "Values in 'genotype_matrix' should all be logical or numeric"
        )
    )
    ...
    assert_that(
        ...
        msg = paste0(
            "'genotype_matrix' must have all zero or FALSE in the diagonal"
        )
    )
    ...
    assert_that(
        ...
        msg = paste0(
            "Row and column names of 'genotype_matrix' must come from Sample_ID(s) found in 'sample_metadata'. Check row/colname(s) ",
...
.validate_anchor_samples <- function(sample_metadata, anchor_samples) {
    ...
    assert_that(
        ...
        msg = paste0(
            "'anchor_samples' contains Sample_ID(s) that have the same Genotype_Group_ID
                   but different Subject_ID(s), check ",
            paste(subject_inconsistent_samples, collapse = ", ")
        )
    )
    assert_that(
        length(genotype_inconsistent_samples) == 0,
        msg = paste0(
            "'anchor_samples' contains Sample_ID(s) that have the same Subject_ID
                   but different Genotype_Group_ID(s), check ",
            paste(genotype_inconsistent_samples, collapse = ", ")
        )
    )
}
