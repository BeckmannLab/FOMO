test_that("solveComprehensiveSearch() is deprecated but matches solveGlobalSearch()", {
    ...
    expect_warning(r1 <- solveComprehensiveSearch(x1), "renamed to 'solveGlobalSearch")
...
test_that("solveComprehensiveSearchFast() is deprecated but matches solveGlobalSearchFast()", {
    ...
    expect_warning(r1 <- solveComprehensiveSearchFast(x1), "renamed to 'solveGlobalSearchFast")
...
test_that("solveEnsemble(use_solvers = 'comprehensive') is a deprecated alias for 'global'", {
    ...
    expect_warning(
        r1 <- solveEnsemble(x1, use_solvers = c("majority", "comprehensive", "local")),
        "deprecated"
    )
    r2 <- suppressMessages(solveEnsemble(x2, use_solvers = c("majority", "global", "local")))
...
test_that("use_solvers rejects mixing 'global' and 'global_fast', including via the deprecated alias", {
    ...
    expect_error(
        suppressWarnings(solveEnsemble(x2, use_solvers = c("majority", "comprehensive", "global_fast"))),
        "cannot contain both"
    )
})
