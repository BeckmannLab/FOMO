---
title: "Introduction to FOMO"
author: "Charles Deng"
package: fomo
output: BiocStyle::html_document
vignette: >
  %\VignetteIndexEntry{FOMOIntro"}
...
### param 3 (optional): label_domains

The researcher has the option to provide the argument label_domains (for label 
domains), which is a data.frame with two columns: one for Sample_ID which 
should match the Sample_ID column in sample_metadata, and another for Label_Domain 
...
sample would be a label domain provided that sample swaps can only 
...
label_domains_input <- data.frame(
    # generally comes from metadata records
    Sample_ID = c("A_1", "A_2", "A_3", "B_1", "B_2", "B_3", "C_1", "C_2", "C_3"),
    # generally comes from metadata records
    Label_Domain = c("WGS", "RNA", "RNA", "RNA", "RNA", "WGS", "RNA", "WGS", "RNA")
)

mislabel_solver <- MislabelSolver(sample_metadata=sample_metadata_input, 
                                  genotype_matrix=genotype_matrix_input,
                                  label_domains=label_domains_input)
...
Node shapes correspond to label domains
...
FOMO provides four solver functions to find likely relabels: solveMajoritySearch, 
solveGlobalSearch, solveLocalSearch, and solveEnsemble. In most cases,
researchers should use the function solveEnsemble, which runs the 
solveGlobalSearch, solveMajoritySearch, and solveLocalSearch in a loop.
...
summary_list <- collateOutput(mislabel_solver_solved)
summary_list$Sample

writeOutput(summary_list, "outputted_corrections.xlsx")
